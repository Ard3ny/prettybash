...
installDepend() {
    ...
    DEPENDENCIES='autojump bash bash-completion tar neovim terminator'
...
createDirs() {
	### Creare dirs for NeoVim and fonts
	mkdir -p ~/.config/nvim
    mkdir ~/.fonts

	
	#download,unzip and move fonts to homedir
	wget -O ~/Downloads/meslo.zip https://github.com/ryanoasis/nerd-fonts/releases/download/v2.3.3/Meslo.zipi
...
linkConfig() {
    ...
    ## Make symbolic link.
    ln -svf ${GITPATH}/.config ${HOME}/.config/terminator/config
